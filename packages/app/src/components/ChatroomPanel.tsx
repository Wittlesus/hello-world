...
export function ChatroomPanel({ fullHeight = false }: { fullHeight?: boolean }) {
  ...
  const [typewriters, setTypewriters] = useState<
    Record<string, { msgId: string; revealed: number }>
  >({});
  ...
  const fetchState = useCallback(async () => {
    ...
    try {
      setState(JSON.parse(await invoke<string>('get_chatroom', { projectPath })) as ChatroomState);
    } catch (err) {
      console.error('[ChatroomPanel] fetchState error:', err);
      setState(null);
    }
  }, [projectPath]);
  ...
  useEffect(() => {
    ...
    state.messages.forEach((m) => {
      if (m.type !== 'system' && m.type !== 'thinking') msgByAgent[m.agentId] = m;
    });
    Object.entries(msgByAgent).forEach(([agentId, msg]) => {
      ...
      if (twIntervals.current[agentId]) {
        clearInterval(twIntervals.current[agentId]);
        delete twIntervals.current[agentId];
      }
      let revealed = 0;
  ...
  useEffect(
    () => () => {
      Object.values(twIntervals.current).forEach(clearInterval);
    },
    [],
  );
  ...
  const logMessages = (state?.messages ?? []).filter(
    (m) => m.type !== 'system' && m.type !== 'thinking',
  );
  ...
  return (
    <>
      ...
      <div
        className={
          fullHeight
            ? 'flex-1 min-h-0 bg-[#09090f] flex flex-col'
            : 'shrink-0 border-t border-gray-800/60 bg-[#09090f] flex flex-col'
        }
        ...
        {!collapsed && (
          <div className="flex flex-1 min-h-0">
            ...
            >
                ...
                {logMessages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.id}>
                      ...
                      <div
                        style={{
                          padding: isNewSpeaker ? '3px 8px 2px' : '1px 8px 2px',
                          lineHeight: 1.45,
              ...
              >
                <span
                  style={{ fontSize: 8, fontFamily: 'monospace', color: '#3a3a58', flexShrink: 0 }}
                >
                  Pat
                </span>
                <span
                  style={{ fontSize: 8, fontFamily: 'monospace', color: '#222235', marginRight: 2 }}
                >
                  ▸
                </span>
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') submitPatMessage();
                  }}
            ...
            <div className="flex-1 relative overflow-hidden" style={{ background: '#030310' }}>
              ...
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    'radial-gradient(ellipse, rgba(255,255,255,0.013) 0%, rgba(255,255,255,0.003) 65%, transparent 100%)',
              ...
              {allBuddies.map((agent) => {
                ...
                // Intro visibility
                const agentOwnIndex = isGolden
                  ? -1
                  : (state?.agents ?? []).findIndex((a) => a.id === agent.id);
                const isRevealed =
                  !inIntro || isGolden || (introCount !== undefined && agentOwnIndex < introCount);
                const isJustRevealed =
                  inIntro &&
                  !isGolden &&
                  introCount !== undefined &&
                  agentOwnIndex === introCount - 1;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      gap: 3,
                      animation: isJustRevealed
                        ? 'agent-enter 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards'
                        : undefined,
                    }}
                  >
                    {/* Speech bubble */}
                    {bubbleTxt !== null && (
                      <div
                        ...
                        style={{
                          ...
                          wordBreak: 'break-word',
                          opacity: bubbleOp,
                          animation: 'bubble-pop 0.12s ease',
                          marginBottom: 2,
                          overflowY: 'auto',
                          pointerEvents: 'auto',
                        }}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      lineHeight: 1,
                      pointerEvents: 'none',
                      animation: 'combat-float 2.4s ease-out forwards',
                      zIndex: 20,
                    }}
                  >
                    {f.emoji}
