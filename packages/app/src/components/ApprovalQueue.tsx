...
function TierBadge({ tier }: { tier: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono uppercase border ${style.bg} ${style.text} ${style.border}`}
    >
      {style.label}
    </span>
  );
...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono uppercase ${style.bg} ${style.text}`}
    >
      {style.label}
    </span>
  );
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {pending.length === 0 ? (
            <p className="text-sm text-gray-600 text-center mt-16 max-w-sm mx-auto">
              No pending approvals. Claude will request approval here before git push, deploy,
              delete, architecture changes, and deliberation recommendations.
            </p>
          ) : (
