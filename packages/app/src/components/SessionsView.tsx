    ...
    <div className="bg-[#1a1a24] border border-gray-800 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          {isActive && (
            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-green-500/20 text-green-300 shrink-0">
              Active
            </span>
          )}
          <div className="flex items-center gap-2 text-xs text-gray-400 flex-wrap">
            ...
            {session.tasksCompleted.length > 0 && (
              <>
                <span className="text-gray-700">·</span>
                <span className="text-blue-400">{session.tasksCompleted.length}t</span>
              </>
            )}
            {session.decisionsMade.length > 0 && (
              <>
                <span className="text-gray-700">·</span>
                <span className="text-purple-400">{session.decisionsMade.length}d</span>
              </>
            )}
          </div>
        </div>
      ...
      {!expanded && session.summary && (
        <div className="px-4 pb-3 text-xs text-gray-500 truncate border-t border-gray-800/40 pt-2">
          {session.summary}
        </div>
      )}
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-gray-800/60">
          {session.summary && (
            <p className="text-xs text-gray-400 py-3 leading-relaxed border-b border-gray-800/40 mb-2">
              {session.summary}
            </p>
          )}
